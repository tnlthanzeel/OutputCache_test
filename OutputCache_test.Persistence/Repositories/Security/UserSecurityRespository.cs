...
namespace OutputCache_test.Persistence.Repositories.Security;
...
public sealed class UserSecurityRespository : BaseRepository, IUserSecurityRespository
{
    ...
    public async Task<ResponseResult<UserDto>> CreateUser(string userName, string password, string email, string role, string firstName, string lastName, IEnumerable<string> permissions, string timeZone, CancellationToken token)
    {
        ...
        UserDto user = new(newUser.Id, newUser.Email, newUser.UserName, firstName, lastName, timeZone, permissionList, new[] { role });
    ...
    public async Task<IReadOnlyList<Role>> GetAllRoles(CancellationToken token, bool asTracking = false, string? searchQuery = null)
    {
        ...
        var allRoles = await query.Where(w => w.Id != AppConstants.SuperAdmin.SuperAdminRoleId && (string.IsNullOrEmpty(searchQuery) || EF.Functions.Like(w.Name!, "%" + searchQuery + "%")))
                                  .OrderBy(o => o.Name)
                                  .ToListAsync(cancellationToken: token);
